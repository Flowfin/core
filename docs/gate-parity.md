...
| `publish-failure-alert.yml` | `Report any workflow that concluded non-success on the default branch` | A scheduled or post-merge workflow that starts failing with nobody waiting on it, and stays red for weeks | satisfied | #90. `.github/workflows/branch-health.yml` runs here under this board's own naming, and `.github/branch-health/branch-health.sh` is the verdict. It takes the property rather than a list: the set it watches is the workflow registry read per run, it opens an issue rather than sending a notification, and it prints every registry entry with what it read of that entry, so a workflow with no run on the default branch is named as one it did NOT examine rather than counted as healthy. Three deviations, each with its reason. The verdict is over each workflow's LATEST conclusion rather than over every run it read, so a leg that failed once and has concluded success since is history rather than a report, and the run still names it under a history heading so the exclusion is a sentence rather than a filter. The read is per workflow rather than one read of the branch, because the branch-wide listing stops handing over rows at a thousand and this branch carries more than a thousand runs in a month, so a paged read of a dated window refuses. And it runs on every pull request as well as on its schedule, because a leg that only runs on its own schedule is the thing this leg exists to catch: a change breaking the script would otherwise be found a day later by a run nobody is waiting on, and a broken script exits before it reports, so it cannot report itself. What it refuses is being unable to produce a report - an unreadable listing, rows in a shape it does not parse, a registry naming nothing - and never a red workflow, which is the condition #26 sets for a reporting leg whose green is worth requiring. Two bounds it prints on every run: it reads a conclusion and never a log, so the reason a run failed is in the run rather than in this report, and it walks the registry, so a workflow file that was deleted keeps its conclusions on the branch and is never asked about. Its issue-opening half is not proven by a fixture, for the reason #90 carries: watching it open one needs a workflow on the default branch that fails on purpose. `.github/workflows/branch-health-lever.yml` is that workflow: a scheduled entry in the same registry, concluding success unless a dispatch asks it for failure, so the step between a derived title and an issue can be watched rather than assumed. It is a workflow rather than a fixture because the subject is the platform accepting the call under that job's token, which is not a property of any text this repository holds, and #90 is where the reading it makes possible is recorded. |
...
| `stryker-mutation.yml` | none, it runs on a schedule and on demand | A suite that covers a line without asserting anything about it | satisfied | #85. `.github/workflows/mutation.yml` runs here on a weekly schedule and on demand, with no deviation in placement: scheduled, reported and gating nothing, because the question it answers is far too slow to sit in front of a merge. What differs is the scope list, since the modules differ, and here it is taken from the `area` lines of `.github/coverage/pinned-surface`, the same register the coverage bar in #84 is pinned to, so the two surfaces cannot come apart and a new file under one of those directories arrives on this scope by being there. Its rules sit in `.github/mutation/mutation.sh` rather than in the workflow file, and each run proves them against their own fixtures before it judges anything: the deciding pair is two crates one assertion apart, identical in every line a test reaches, which score 100 and 0. What a falling score obliges is written there and printed beside every score, and it is an issue per survivor carrying the line the run printed, not a red gate. There is no suppression register on this leg, unlike the two analysers beside it, because excusing a mutant raises the published number without any test being written. Three bounds it prints on every run: nothing outside those areas carries a mutant, a caught mutant says the suite reacted and never which assertion reacted, and the two targets `Cargo.toml` keeps out of `cargo test` are not tests any mutant was put in front of. |
