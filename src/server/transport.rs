...
/// so a tile nobody is looking at any more closes its connection rather than
/// downloading in full to save a handshake. The word 0027 uses there is one the
/// `no-view-vocabulary` rule refuses inside `src/`, which is that rule working:
/// a record may describe a screen and this core may not learn its words.
